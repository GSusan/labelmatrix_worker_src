...
# Project-specific font directory (relative to project root)
PROJECT_FONT_DIR = ROOT / "fonts" if ROOT else Path(__file__).parent.parent.parent / "fonts"
...
def check_font(font="Arial.ttf"):
    ...
    # 1. Check project font directory FIRST (for offline deployment)
    if PROJECT_FONT_DIR.exists():
        file = PROJECT_FONT_DIR / name
        if file.exists():
            return file

    # 2. Check USER_CONFIG_DIR
    ...
    # 4. Try to download to USER_CONFIG_DIR if missing (may fail in offline mode)
    ...
    # 5. If all else fails, return the project font path anyway (will fail if not exists)
    if PROJECT_FONT_DIR.exists():
        return PROJECT_FONT_DIR / name

    raise FileNotFoundError(f"Font file not found: {font}. Please place it in {PROJECT_FONT_DIR}")
...
def check_amp(model):
    ...
    try:
        ...
        # 优先从 labelmatrix_integration/pretrained_models 目录加载模型
        pretrained_models_dir = Path(__file__).parent.parent.parent / "labelmatrix_integration" / "pretrained_models"
        model_path = pretrained_models_dir / "yolo26n.pt"
        if not model_path.exists():
            model_path = "yolo26n.pt"  # 回退到默认下载

        assert amp_allclose(YOLO(str(model_path)), im)
